...
/**
 * Validates board assembly, cell flipping, and victory detection provided by {@link
 * BoardAssembler}.
 */
class BoardAssemblerTests {
  ...
  void assembledBoardHasTwentyFiveCells() {
    ...
    assertEquals(25, generatedBoard.size(), "A standard bingo grid must hold 25 tiles");
  }
  ...
  void nonFreeCellsStartUnselected() {
    ...
    for (BingoCell tile : generatedBoard) {
      if (!tile.freeCell()) {
        assertFalse(tile.selected(), "Tile id=" + tile.id() + " should begin unselected");
      }
    }
  }
  ...
  void flippingCellTogglesSelection() {
    ...
    assertTrue(
        afterFirstFlip.get(targetId).selected(), "First flip should mark the cell as selected");

    List<BingoCell> afterSecondFlip = BoardAssembler.flipCell(afterFirstFlip, targetId);
    assertFalse(
        afterSecondFlip.get(targetId).selected(),
  ...
  void flippingFreeCellHasNoEffect() {
    ...
    assertTrue(
        afterAttemptedFlip.get(freeCellId).selected(),
        "Free cell must remain selected even after a flip attempt");
    assertTrue(afterAttemptedFlip.get(freeCellId).freeCell(), "Free cell flag must not change");
  }
  ...
  void completeRowDetectedAsVictory() {
    ...
    assertEquals("row", result.get().direction(), "The detected streak direction should be 'row'");
    assertEquals(0, result.get().index(), "The winning row index should be 0");
  }
  ...
  void incompleteBoardHasNoVictory() {
    ...
    assertTrue(result.isEmpty(), "No streak should be found on a brand-new board");
  }
