...
/**
 * Pure-logic helper that builds boards, flips tiles, and spots victories. Every method is static —
...
public final class BoardAssembler {
  ...
  public static Optional<WinningStreak> detectWinningStreak(List<BingoCell> board) {
    ...
    List<Integer> diagMain =
        IntStream.range(0, GRID_SIDE).map(i -> i * GRID_SIDE + i).boxed().toList();
    if (allSelected(board, diagMain)) {
      return Optional.of(new WinningStreak("diagonal", 0, diagMain));
    }

    // Anti-diagonal (top-right → bottom-left)
    List<Integer> diagAnti =
  ...
  private static List<Integer> positionsForRow(int row) {
    return IntStream.range(0, GRID_SIDE).map(col -> row * GRID_SIDE + col).boxed().toList();
  }

  private static List<Integer> positionsForColumn(int col) {
    return IntStream.range(0, GRID_SIDE).map(row -> row * GRID_SIDE + col).boxed().toList();
  }
